"""
face_blend_v9_1_final.py

Stable, production-safe face overlay

Key Design:
- NO perspective warp (removed unstable behavior)
- Pose affects POSITION (not texture distortion)
- Per-page config works correctly
"""
...
def process_scene(template_path, user_img, page_number):
    ...
    det_yaw, det_pitch, det_roll = rotation_to_euler(
        estimate_head_pose(landmarks, user_img.shape)
    )
    ...
    yaw = np.clip(det_yaw + json_yaw, -15, 15)
    pitch = np.clip(det_pitch + json_pitch, -10, 10)
    roll = np.clip(det_roll + json_roll, -25, 25)

    # 🔥 KEY FIX: Pose affects POSITION
    pose_dx = int(yaw * 2.5)
    pose_dy = int(pitch * 2.0)

    # reduced anchor correction
    anchor_dx = int((0.5 - nx) * fw * 0.2)
    anchor_dy = int((0.45 - ny) * fh * 0.2)

    dx = pose_dx + anchor_dx
    dy = pose_dy + anchor_dy
    ...
    print(f"[Page {page_number}] yaw={yaw:.2f}, pitch={pitch:.2f}, roll={roll:.2f}")
